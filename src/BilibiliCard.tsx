...
export function BilibiliCard({onConnectionChange}: {onConnectionChange?: (connected: boolean) => void} = {}) {
  ...
  return <section className="platform-card bilibili">
    ...
    {error && <p className="inline-error" role="alert">{error} <button type="button" className="text-button" onClick={()=>{setError("");setQrOpen(true);}}>重新登录</button></p>}
    ...
    {qrOpen && createPortal(<BilibiliQrModal onClose={() => setQrOpen(false)} onConnected={() => {setSaved(true); onConnectionChange?.(true); setQrOpen(false);}}/>, document.body)}
  </section>;
}
